...
class Models_Evolution():
    ...
    def F81(self, frequency_A, frequency_G, frequency_C, frequency_T):
        ...
        return intensity_matrix
    def GTR(self, A_to_G, A_to_C, A_to_T, G_to_C, G_to_T, C_to_T, frequency_A, frequency_G, frequency_C, frequency_T):
        A_to_A = -1 * (A_to_G*frequency_G + A_to_C*frequency_C + A_to_T*frequency_T)
        T_to_T = -1 * (A_to_G*frequency_A + G_to_C*frequency_C + G_to_T*frequency_T)
        C_to_C = -1 * (A_to_C*frequency_A + G_to_C*frequency_G + C_to_T*frequency_T)
        G_to_G = -1 * (A_to_T*frequency_A + G_to_T*frequency_G + C_to_T*frequency_C)
        intensity_matrix = np.array([  [A_to_A,             A_to_G*frequency_G, A_to_C*frequency_C, A_to_T*frequency_T],
                                       [A_to_G*frequency_A, G_to_G,             G_to_C*frequency_C, G_to_T*frequency_T],
                                       [A_to_C*frequency_A, G_to_C*frequency_G, C_to_C,             C_to_T*frequency_T],
                                       [A_to_T*frequency_A, G_to_T*frequency_G, C_to_T*frequency_C, T_to_T]])
        return intensity_matrix
    def generate_random_intensity_matrix(self, rand_value='True', print_datas=True):
        ...
        if rand_value == 0:
            ...
            params = [1/random.randint(1,100),1/random.randint(1,100),1/random.randint(1,100),1/random.randint(1,100),1/random.randint(1,100),1/random.randint(1,100), 1/random.randint(1,100),1/random.randint(1,100),1/random.randint(1,100),1/random.randint(1,100)]
            if print_datas:
                print(f'Start model: GTR, with params = {params}')
            intensity_matrix = self.GTR(*params)
